...
Terraform import brings an **existing resource** under Terraform state management. It does not create the cloud resource and it does not automatically write a matching `resource` block for you. After an import, Terraform can compare the resource in state with the configuration and manage future changes.

### When to Use Imports
- Adopt infrastructure created manually, by a script, or by another tool
- Move a resource into a Terraform-managed environment gradually
- Recover state after confirming the real resource still exists

### Safe Import Workflow

1. Find the provider's resource type and the resource's import ID format in its documentation.
2. Write a `resource` block at the address Terraform should manage, matching the intended final configuration.
3. Run `terraform init` and then import the existing resource.
4. Run `terraform plan`; adjust the configuration until the plan shows no unwanted changes.
5. Apply only after carefully reviewing the plan.

For example, importing an existing AWS EC2 instance with ID `i-0123456789abcdef0`:

```hcl
# main.tf
resource "aws_instance" "web" {
  # Add the arguments needed to match the existing instance.
  # For example: ami, instance_type, subnet_id, tags, etc.
}
```
...
The address (`aws_instance.web`) is the Terraform state address; the final value is the provider-specific resource ID. Import IDs differ by provider and sometimes use compound values such as `project/name`.

### Declarative Import Blocks

Import blocks make imports reviewable in version control and are useful when importing several resources:
...
Run `terraform plan` and `terraform apply` to perform the import. Once it succeeds, the import block can normally be removed; the resource remains tracked in state.

### Important Notes
- Importing changes Terraform **state**, not the remote infrastructure.
- A remote object must be managed by only one Terraform resource address/state at a time.
- `terraform import` imports one resource at a time; dependencies are not imported automatically.
- Back up shared state and use a remote backend with locking before production imports.
- Always use `terraform plan` after an import—missing or different arguments can otherwise cause Terraform to propose updates or replacement.
...
A module is a collection of Terraform files in one directory. Every Terraform configuration has a **root module** (the directory where you run Terraform); reusable directories called by it are **child modules**. Modules package related resources behind inputs and outputs, so the same infrastructure pattern can be used consistently across environments.

### What Modules Help You Do
- Reuse standard patterns such as VPCs, networks, EC2 instances, or Kubernetes clusters
- Keep root configurations small and focused on environment-specific values
- Enforce common tags, naming, security settings, and defaults
- Version shared infrastructure code and upgrade it deliberately
...
The calling module passes input variables; it reads values exposed by the child only through outputs. Resources inside this module have addresses such as `module.network.aws_vpc.this`.

### Module Sources and Versioning

Modules can come from a local path, the Terraform Registry, Git, or an archive. Pin versions for remote modules so an update is intentional:
...
Run `terraform init` whenever adding or changing a module source. Use `terraform get -upgrade` to fetch newer allowed module versions, then review the resulting plan.

### Module Design Guidelines
- Keep each module focused on one clear responsibility.
- Use typed, documented variables and sensible defaults; validate important inputs.
- Expose useful outputs, but avoid exposing every internal resource.
